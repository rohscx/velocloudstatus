module.exports =  function (data){
    return new Promise((resolve,reject) => {
        ...
        if (!vcoAuthResponse) reject({rejected:["enterpriseId.js","bad data","const vcoAuthResponse = data;"]});
        const authResponse = vcoAuthResponse.map((data) => {
            //console.log(data)
            const {metaData, options} = data;
            const {auth, symbol, routeHistory, sessionId} = metaData;
            const route = 'https://vco18-usvi1.velocloud.net/portal/rest/enterprise/getEnterprise';
