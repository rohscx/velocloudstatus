...
const data = baseData.map(({enterprise:{enterpriseEdge,enterpriseName}}) => ({enterpriseEdge,enterpriseName}));

const result = data.map(({enterpriseEdge,enterpriseName}) => enterpriseEdge[0].map(({name, modelNumber}) =>({ enterpriseName,name,
    modelNumber})))//?
